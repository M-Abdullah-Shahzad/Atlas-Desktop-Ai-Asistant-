...
def _normalize_update_keys(updates: dict, *, top_level: bool = True) -> dict:
    ...
    for key, value in updates.items():
        key_text = str(key or "")
        keep = top_level and (key_text in _empty_memory() or key_text == "sessions")
        nk = key_text if keep else _snake_case(key_text)
...
def set_response_language(language: str) -> str:
    ...
    global _session_response_language
    val = str(language or "").strip() or "English"
    with _lock:
...
def peek_last_session() -> dict | None:
    ...
    with _lock:
        ...
        try:
            ...
            print(f"[Memory] WARNING: peek_last_session error: {e}")
            return None
...
def pop_last_session() -> dict | None:
    ...
    with _lock:
        ...
        try:
            ...
            print(f"[Memory] WARNING: pop_last_session error: {e}")
            return None
