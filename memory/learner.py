...
def _ensure_loaded() -> None:
    ...
    if _activity is None:
        ...
        days = _activity.get("days")
        if not isinstance(days, dict):
            _activity["days"] = {}
        else:
            _activity["days"] = {
                str(day): hours if isinstance(hours, dict) else {}
                for day, hours in days.items()
            }
    if _experience is None:
        _experience = _load_json(_experience_path(), _empty_experience())
        events = _experience.get("events")
        counts = _experience.get("tool_counts")
        _experience["events"] = events if isinstance(events, list) else []
        _experience["tool_counts"] = counts if isinstance(counts, dict) else {}
...
def tick() -> None:
    ...
    with _lock:
        ...
        if not isinstance(hour_map, dict):
            hour_map = {}
            day_map[hour] = hour_map
        try:
            previous = float(hour_map.get(exe, 0.0))
        except (TypeError, ValueError):
            previous = 0.0
        hour_map[exe] = previous + _TICK_INTERVAL_MIN
        _dirty = True
        _flush_if_needed()
...
def log_tool(tool: str, action: str = "", outcome: str = "ok") -> None:
    ...
    outcome = str(outcome or "ok").strip().lower()
    ...
    with _lock:
        ...
        if not isinstance(counts, dict):
            counts = {}
            _experience["tool_counts"] = counts
        try:
...
def looks_like_correction(text: str) -> bool:
    return bool(text and _CORRECTION_RE.search(str(text)))
