...
# Windows cp1252 consoles still crash on characters like '→' or emoji.
# Normalize to the active terminal encoding before writing, with replacement fallback.
def _console_safe_text(value: object) -> str:
    text = "" if value is None else str(value)
    enc = getattr(sys.stdout, "encoding", None) or getattr(sys.stderr, "encoding", None) or ""
    target = (enc or "").lower()
    if not target or "utf" in target:
        return text
    try:
        return text.encode(target, errors="replace").decode(target)
    except Exception:
        for alt in ("cp1252", "latin-1", "ascii"):
            try:
                return text.encode(alt, errors="replace").decode(alt)
            except Exception:
                continue
        return text.encode("ascii", errors="replace").decode("ascii")

_original_print = __import__("builtins").print

def _safe_print(*args, sep=" ", end="\n", file=None, flush=False):
    stream = file if file is not None else sys.stdout
    safe_args = tuple(_console_safe_text(a) for a in args)
    return _original_print(*safe_args, sep=sep, end=end, file=stream, flush=flush)

__import__("builtins").print = _safe_print
...
def _transcript_for_log(text: str) -> str:
    """Return the recognized input unchanged for the Activity Log."""
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "'Composed by Atlas' and does NOT send. Pass the spoken saved name "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "topic": {"type": "STRING", "description": "Brief topic for automatic detailed Roman Urdu message generation"},
    ...
    },
    {
        ...
        "description": (
            "Initiates an audio or video call through the linked WhatsApp Baileys bridge. "
            "Resolve the saved contact name exactly and report the backend result; never claim "
            "a call started unless the tool returns ok."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "contact_name": {"type": "STRING", "description": "Saved WhatsApp contact name"},
                "call_type": {"type": "STRING", "description": "audio or video (default audio)"},
            },
    ...
    },
    {
        "name": "gmail_control",
        "description": (
            "Reads Gmail through the Gmail API. Use action=read when the user asks "
            "to read, list, or filter emails, including emails after a time such as 5 PM. "
            "Use action=login to open Gmail OAuth. The tool returns message metadata and snippets; "
            "do not claim access until the result confirms it."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {"type": "STRING", "description": "read | login | status"},
                "after": {"type": "STRING", "description": "Optional time/date filter, e.g. 'today 5 PM' or ISO timestamp"},
                "limit": {"type": "INTEGER", "description": "Maximum emails to return, default 20"},
            },
            "required": ["action"],
        },
    },
    {
        ...
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":       {"type": "STRING", "description": "compose (default) | send | abort | login"},
    ...
    {
        "name": "auto_reply_gmail",
        "description": (
            "Sends a threaded Gmail reply to an existing message using the Gmail API. "
            "Use the message ID, recipient email, and exact reply text."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "message_id": {"type": "STRING", "description": "Gmail message ID to reply to"},
                "recipient_email": {"type": "STRING", "description": "Reply recipient email address"},
                "reply_text": {"type": "STRING", "description": "Reply body"},
            },
            "required": ["message_id", "recipient_email", "reply_text"],
        },
    },
    {
        "name": "set_gmail_vacation_responder",
        "description": (
            "Turns Gmail's global Vacation Responder on or off for incoming mail. "
            "Use this for requests like auto-reply to everyone, not for replying to one thread."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                ...
                "response_body": {"type": "STRING", "description": "Global auto-reply text"},
                "restrict_to_contacts": {"type": "BOOLEAN", "description": "Reply only to contacts"},
                "restrict_to_domain": {"type": "BOOLEAN", "description": "Reply only to the domain"},
            },
            "required": ["enabled"],
        },
    },
    ...
    },
    {
        ...
        "description": "Creates and runs lightweight Atlas agent tasks for coding, meeting, and job workflows without replacing the existing tool architecture.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "agent": {"type": "STRING", "description": "coding | meeting | job"},
                "request": {"type": "STRING", "description": "The user's request or brief for the selected agent"},
    ...
    },
    {
        ...
        "description": (
            ...
            "Atlas checks these topics once a day and alerts when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            "Control Atlas's continuous PC monitoring, proactive notifications, and habit learning. "
    ...
    },
    {
        ...
        "description": (
            "Puts Atlas to sleep: hides the HUD window and keeps the process "
    ...
    },
    {
        ...
        "description": (
            "Fully quits Atlas and exits the process. "
            "Call ONLY when the user explicitly wants to shut down completely, "
            "quit Atlas, or end the program forever — e.g. 'quit Atlas', "
...
class AthenaLive:
    ...
    def __init__(self, ui: AthenaUI):
        ...
        athena_log(f"Atlas logger ready → {log_path()}")
        try:
    ...
    def _start_wake_listener(self) -> None:
        try:
            ...
            self.ui.write_log("SYS: Use the tray icon to show Atlas.")
    ...
    def _arm_pending_permission(self, name: str, args: dict, decision) -> None:
        ...
        print(f"[ATLAS] ⏳ permission required: {decision.summary}")
        if self._dashboard:
    ...
    async def _resolve_voice_permission(self, verdict: str, user_text: str = "") -> None:
        ...
        if verdict == "deny":
            ...
            print(f"[ATLAS] 🚫 voice abort: {summary}")
            try:
        ...
        print(f"[ATLAS] ✅ voice grant: {summary}")
    ...
    def interrupt(self) -> None:
        """Stop Atlas mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[ATLAS] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name != "save_memory" and not local_grant:
            ...
            if decision.requires_confirmation:
                ...
                if preapproved:
                    print(f"[ATLAS] ✅ voice-granted {name}")
        ...
        try:
            if name == "open_app":
            ...
            elif name == "initiate_whatsapp_call":
                from actions.whatsapp_bridge_client import initiate_whatsapp_call
                result = await asyncio.to_thread(
                    initiate_whatsapp_call,
                    str(args.get("contact_name") or ""),
                    call_type=str(args.get("call_type") or "audio"),
                )
                ...
                from actions.gmail_bridge_client import read_emails, run_oauth_flow, status as gmail_status
                action = str(args.get("action") or "status").strip().lower()
                if action == "login":
                    result = await asyncio.to_thread(run_oauth_flow, timeout=180.0, open_browser=True)
                elif action == "read":
                    result = await asyncio.to_thread(
                        read_emails,
                    ...
                    if isinstance(result, dict) and result.get("ok"):
                        lines = [f"Gmail messages found: {result.get('count', 0)}"]
                        ...
                        text = "\n\n".join(lines)
                        self.ui.show_content("GMAIL", text)
                        result = text
                elif action == "status":
                    result = gmail_status()
                else:
                    result = "Use Gmail action read, login, or status."

            elif name == "auto_reply_gmail":
                from actions.gmail_bridge_client import auto_reply_gmail
                result = await asyncio.to_thread(
                    auto_reply_gmail,
                    message_id=str(args.get("message_id") or ""),
                    recipient_email=str(args.get("recipient_email") or ""),
                    reply_text=str(args.get("reply_text") or ""),
                )

            elif name == "set_gmail_vacation_responder":
                from actions.gmail_bridge_client import set_gmail_vacation_responder
                result = await asyncio.to_thread(
                    set_gmail_vacation_responder,
                    enabled=bool(args.get("enabled")),
                    response_subject=str(args.get("response_subject") or "Out of office"),
                    response_body=str(args.get("response_body") or ""),
                    restrict_to_contacts=bool(args.get("restrict_to_contacts")),
                    restrict_to_domain=bool(args.get("restrict_to_domain")),
                )
                ...
                if _cs_action in ("sleep", "sleep_pc", "shutdown", "restart"):
                    ...
                    async def _deferred_pc_power(_a=_cs_action, _p=_cs_args):
                        ...
                        try:
                            ...
                            print(f"[ATLAS] deferred PC {_a} failed: {e}")
                ...
                agent = str(args.get("agent") or "coding").strip().lower()
                request = str(args.get("request") or "").strip()
                if not request:
                    result = "Agent request is empty. Please provide a brief for the selected agent."
                else:
                    orchestrator = AgentOrchestrator()
                    task = orchestrator.create_task(agent, request, str(args.get("task_id") or None))
                    task_data = orchestrator.run_task(task)
                    result = (
                        f"Agent task {task.task_id} for {agent} completed. "
    ...
    async def _receive_audio(self):
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_in:
                                log_in = _transcript_for_log(full_in)
                                self.ui.write_log(f"You: {log_in}")
                                self._session_log.append(f"User: {log_in}")
                                try:
                                    if looks_like_correction(full_in):
                                        prev = ""
                                        for line in reversed(self._session_log[:-1]):
                                            if line.startswith(f"{self._asst_name}:") or line.startswith("Atlas:"):
            ...
            if _transient:
                print("[ATLAS] Gemini Live connection dropped (server error) — will reconnect.")
    ...
    async def _send_wake_greeting(self) -> None:
        ...
        if not self.session or self._hud_sleeping:
            return
        name = self.ui.assistant_name or self._asst_name or "Atlas"
        prompt = (
    ...
    async def run(self):
        ...
        while True:
            ...
            try:
                ...
                if self._is_sleep_exc(e):
                    ...
                    if self._reconnect_needed:
                        print("[ATLAS] Reconnecting after API/model change.")
                        self._reconnect_needed = False
                        if self._sleep_request is not None:
                            self._sleep_request.clear()
                    else:
                        print("[ATLAS] Sleep requested — pausing Gemini.")
                else:
                    err_str = str(e)
                    _transient = (
                        "1011" in err_str
                    ...
                    if _transient:
                        print("[ATLAS] Gemini Live server error — reconnecting.")
                        self.ui.write_log("SYS: Connection dropped. Reconnecting…")
                        self._conn_backoff = 3
                    else:
                        print(f"[ATLAS] Error ({type(e).__name__}): {e}")
                        traceback.print_exc()

                    # Invalid/expired credentials — stop hammering the API and
                    # prompt for a replacement key instead of showing SLEEPING.
                    _auth_err = err_str.lower()
                    if (
                        "api key not valid" in _auth_err
                        or "invalid authentication credentials" in _auth_err
                        or "1007" in _auth_err
                        or "1008" in _auth_err
                    ):
                        ...
                        print("[ATLAS] New API key saved — reconnecting...")
                        _conn_backoff = 3
                        continue
            ...
            print(f"[ATLAS] Reconnecting in {delay}s...")
...
def main():
    ...
    face = None
    for candidate in (
        BASE_DIR / "config" / "astra.png",
        BASE_DIR / "config" / "astra.ico",
