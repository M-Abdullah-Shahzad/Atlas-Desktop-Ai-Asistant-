...
PROJECTS_DIR     = Path.home() / "Desktop" / "AthenaProjects"
MAX_FIX_ATTEMPTS = 5
...
GROQ_MODEL = GROQ_FALLBACK_MODELS[0]
...
def _get_model(model_name: str):
    ...
    if not api_key:
        raise RuntimeError(
            "GROQ_API_KEY is not configured. Set it in the environment or the project .env file."
        )
    ...
    class _W:
        def generate_content(self, contents):
            last_error = None
            models = list(dict.fromkeys(GROQ_FALLBACK_MODELS))
                ...
                try:
                    ...
                    if not _is_model_fallback_error(error):
                        ...
                        for attempt, delay in enumerate(API_RETRY_DELAYS):
                            print(
                                f"[DevAgent] Waiting for API capacity... retrying in "
                                f"{delay}s ({attempt + 1}/{len(API_RETRY_DELAYS)})"
                            )
                            time.sleep(delay)
                            try:
                                response = client.chat.completions.create(
                                ...
                                if not _is_retryable_api_error(retry_error):
                                    raise
                        # Capacity may be specific to this model; continue with
                        # the next configured model after the backoff attempts.

                    if model != models[-1]:
                        next_model = models[models.index(model) + 1]
                        print(
                            f"[DevAgent] Warning: Groq model '{model}' failed "
                            f"({error}); trying fallback model '{next_model}'."
                        )

            raise RuntimeError(
                f"All configured Groq models failed after trying {len(models)} models."
            ) from last_error
...
def _is_rate_limit(error: Exception) -> bool:
    return _is_retryable_api_error(error)


def _is_retryable_api_error(error: BaseException) -> bool:
    """Return true for transient Groq capacity and rate-limit failures."""
...
def _is_model_fallback_error(error: BaseException) -> bool:
    """Return true when Groq rejected the selected model or is overloaded."""
    status = getattr(error, "status_code", None) or getattr(error, "code", None)
    if str(status) == "503":
        return True
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "Athena_project")
