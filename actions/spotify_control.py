...
def _spotify_connected() -> bool:
    """Return True only when the Spotify account is reachable and authenticated."""
    ...
    if isinstance(data, dict):
        err = str((data.get("error") or {}).get("message") or "").lower()
...
def _auth_url(client_id: str, redirect_uri: str, challenge: str, state: str) -> str:
    ...
    if parse_qs(urlparse(url).query).get("response_type") != ["code"]:
        raise RuntimeError("Spotify authorization URL must include response_type=code.")
    return url
...
def _set_pending_oauth(client_id: str, verifier: str, redirect_uri: str, state: str) -> None:
...
def _complete_pending_oauth(redirect_url: str) -> str:
    """Complete a pending PKCE login from a pasted Spotify callback URL."""
    with _oauth_pending_lock:
        pending = dict(_oauth_pending or {})
    if not pending:
        raise RuntimeError("No Spotify login is waiting for a callback. Start login again.")
    ...
    if error:
        ...
        raise RuntimeError(f"Spotify auth error: {error}")
    code = (query.get("code") or [None])[0]
    state = (query.get("state") or [None])[0]
    if not code:
        raise RuntimeError("The pasted URL does not contain a code= parameter.")
    if state != pending["state"]:
        raise RuntimeError("OAuth state mismatch. Paste the complete callback URL from this login attempt.")
    ...
    _clear_pending_oauth()
    global _auth_bg_error
    _auth_bg_error = ""
    return "Spotify linked. You can use voice controls now."


def complete_spotify_login(redirect_url: str) -> str:
    """Public manual fallback for a pasted http://127.0.0.1 callback URL."""
    return _complete_pending_oauth(redirect_url)


def _manual_callback_prompt() -> str | None:
    if not getattr(sys.stdin, "isatty", lambda: False)():
        return None
    print(
        "Paste the full Spotify callback URL from the browser address bar "
        "(it should contain code=), then press Enter."
    )
    try:
        value = input("Spotify callback URL: ").strip()
    except (EOFError, KeyboardInterrupt):
        return None
    return value or None


def _run_oauth_flow(*, timeout: float = 300.0, open_browser: bool = True) -> str:
    ...
    _set_pending_oauth(client_id, verifier, redirect_uri, state)
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802
            ...
            body = (
                b"<html><body><h2>Atlas Spotify linked.</h2>"
    ...
    try:
        ...
        bind_error = f"Could not bind Spotify callback listener on 127.0.0.1:{port}: {exc}"
    if open_browser:
        if not _open_in_browser(url):
            raise RuntimeError(
                "Could not open the Spotify authorization browser. "
    ...
    if server is None:
        manual_url = _manual_callback_prompt()
        if manual_url:
            return _complete_pending_oauth(manual_url)
        raise RuntimeError(
            f"{bind_error} Paste the full browser callback URL containing code= "
            "to complete login."
        )
    ...
    if not box["code"]:
        ...
        if manual_url:
            return _complete_pending_oauth(manual_url)
        detail = f" {bind_error}" if bind_error else ""
        raise RuntimeError(
            "Spotify login timed out or the callback listener was unavailable."
            f"{detail} Copy the full browser callback URL containing code= and "
            "complete it with spotify_control(action='login_callback', "
            "redirect_url=<url>)."
        )
    if box["state"] != state:
        _clear_pending_oauth()
...
def _start_background_auth() -> None:
    ...
    global _auth_bg_started, _auth_bg_error
    with _auth_bg_lock:
        if _auth_bg_started:
            return
    ...
    def _worker():
        global _auth_bg_started, _auth_bg_error
        try:
            _run_oauth_flow(timeout=300.0, open_browser=True)
        except Exception as exc:
            _auth_bg_error = str(exc)
        finally:
            with _auth_bg_lock:
                _auth_bg_started = False
...
def _auth_status() -> str | None:
    ...
    if not str(cfg.get("refresh_token") or "").strip():
        ...
        detail = f" {_auth_bg_error}" if _auth_bg_error else ""
        return (
            "AUTH_REQUIRED: Finish Spotify login in the browser that just opened "
            f"({_login_hint()}).{detail}"
        )
    if not _get_access_token():
        _start_background_auth()
        return (
...
def spotify_control(
    ...
    try:
        if action in {"login_callback", "complete_login", "manual_login"}:
            callback = str(params.get("redirect_url") or params.get("callback_url") or query).strip()
            if not callback:
                return "Paste the full Spotify callback URL containing code= to finish login."
            return complete_spotify_login(callback)
        ...
        if not _spotify_connected():
            return "Spotify is not connected. Open Spotify, sign in, and try again."
...
def main() -> None:
    ...
    parser.add_argument(
        "--callback",
        default="",
        help="Complete a pending login using the full pasted Spotify callback URL",
    )
    ...
    if args.login:
        try:
            print(_run_oauth_flow(timeout=300.0, open_browser=True))
        except Exception as e:
    ...
    if args.callback:
        try:
            print(complete_spotify_login(args.callback))
        except Exception as e:
            print(f"Callback completion failed: {e}", file=sys.stderr)
            sys.exit(1)
        return
