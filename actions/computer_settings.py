...
_VOLUME_PRESETS = {
    "max": 100, "maximum": 100, "full": 100, "loudest": 100, "highest": 100,
    "mid": 50, "middle": 50, "medium": 50, "half": 50,
    "low": 15, "quiet": 15, "soft": 15, "min": 15, "minimum": 15,
}

_VOLUME_ACTION_LEVELS = {
    "volume_max": 100, "max_volume": 100, "volume_to_max": 100,
    "set_volume_max": 100, "volume_maximum": 100, "maximum_volume": 100,
    "volume_mid": 50, "mid_volume": 50, "volume_to_mid": 50,
    "volume_medium": 50, "medium_volume": 50, "volume_to_medium": 50,
    "volume_low": 15, "low_volume": 15, "volume_to_low": 15,
    "set_volume_low": 15,
}

_VOLUME_BUMP_ACTIONS = {
...
def _parse_volume_value(value) -> int | None:
    ...
    if s in _VOLUME_PRESETS:
        return _VOLUME_PRESETS[s]
    try:
...
def _volume_from_text(text: str) -> int | None:
    ...
    if "volume" not in t:
        if t in _VOLUME_PRESETS:
            return _VOLUME_PRESETS[t]
        return None
    if re.search(r"\b(max|maximum|full|loudest|highest)\b", t):
        return 100
    if re.search(r"\b(mid|middle|medium|half)\b", t):
        return 50
    if re.search(r"\b(low|quiet|soft|min|minimum)\b", t):
        return 15
    m = re.search(r"\b(?:to|at|=)?\s*(\d{1,3})\s*%?\b", t)
    if m:
        return max(0, min(100, int(m.group(1))))
    return None


def _resolve_volume_level(action: str, value, description: str) -> int | None:
    action_n = (action or "").lower().strip().replace(" ", "_").replace("-", "_")
    desc_level = _volume_from_text(description or "")
    if desc_level is not None:
        return desc_level
    if action_n in _VOLUME_ACTION_LEVELS:
        return _VOLUME_ACTION_LEVELS[action_n]
    if action_n in _VOLUME_BUMP_ACTIONS:
        return None
    parsed = _parse_volume_value(value)
    if action_n in ("volume_set", "set_volume", "set_volume_level", "volume"):
        return parsed
    if "volume" in action_n:
        return _volume_from_text(action_n.replace("_", " ")) or parsed
    if not action_n:
        return parsed
    return None
...
def sleep_computer():
    if _OS == "Windows":
        try:
            # Hibernate/sleep via SetSuspendState (0 = sleep)
            subprocess.run(
            ...
            try:
                ...
                print(f"[Settings] sleep failed: {e}")
    elif _OS == "Darwin":
...
def close_application_by_name(app_name: str) -> str:
    """Close processes matching a friendly application name."""
    ...
    for proc in psutil.process_iter(["pid", "name"]):
        try:
            ...
            if needle in pname or pname in needle:
                proc.terminate()
                killed.append(proc.info.get("name") or str(proc.info.get("pid")))
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue

    if not killed:
        # Fallback: Alt+F4 after trying to focus
        try:
            from actions.computer_control import _focus_window
            _focus_window(name)
            time.sleep(0.3)
            close_app()
            return f"Sent close to window matching '{name}'."
        except Exception:
            return f"No running process matched '{name}'."
    return f"Closed: {', '.join(killed[:8])}."
...
ACTION_MAP: dict[str, callable] = {
    ...
    "sleep_pc":            sleep_computer,
}

# Dangerous actions are gated by core.permissions + HUD — no model self-confirm
_DANGEROUS_ACTIONS = {"restart", "shutdown", "sleep", "sleep_pc"}
...
- Volume presets: "max"/"maximum"/"turn volume to max" → volume_set 100;
  "mid"/"medium"/"turn volume to mid" → volume_set 50;
  "low"/"turn volume to low" → volume_set 15.
  Never use volume_up or volume_down for these.
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp = _client.models.generate_content(model=get_flash_lite_model(), contents=prompt)
...
def computer_settings(
    ...
    def _apply_volume(level: int) -> str:
        print(f"[Settings] Action: volume_set  Value: {level}  OS: {_OS}")
        if player:
            player.write_log(f"[Settings] volume_set {level}")
        try:
            volume_set(level)
    ...
    volume_level = _resolve_volume_level(raw_action, value, description)
    if volume_level is not None:
        return _apply_volume(volume_level)
    ...
    if action in _DANGEROUS_ACTIONS:
        # Confirmation is handled by core.permissions + HUD in main.py
        pass

    if action in ("close_application", "close_app_by_name", "close_named"):
        app = str(value or params.get("app_name") or params.get("name") or "").strip()
        if not app and description:
            # crude: last words after "close"
            app = description
            for prefix in ("close ", "quit ", "kill ", "kapat "):
                if app.lower().startswith(prefix):
                    app = app[len(prefix):]
                    break
        return close_application_by_name(app)
