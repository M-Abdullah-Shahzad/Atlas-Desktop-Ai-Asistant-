...
def send_message(
    ...
    if action in ("login", "auth", "authenticate") and platform == "gmail":
        from actions.gmail_bridge_client import run_oauth_flow
        try:
            return run_oauth_flow(timeout=180.0, open_browser=True)
        except Exception as e:
