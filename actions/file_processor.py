"""
file_processor.py — Athena Universal File Processor
...
def _clip(text: str, limit: int = _MAX_RESULT) -> str:
    t = text or ""
    if len(t) <= limit:
        return t
    return t[:limit] + f"\n\n[Truncated — {len(t)} total chars]"
...
def _gemini_client():
    ...
    class _W:
        def generate_content(self, contents):
            return _c.models.generate_content(model=get_flash_model(), contents=contents)
        ...
        if file_type == "csv":
            ...
            for enc in ("utf-8-sig", "utf-8", "utf-16", "cp1252", "latin-1"):
                try:
                    ...
                    df = None
            if df is None:
                return f"Could not read file: {last_err}"
        else:
...
def file_processor(parameters: dict, player=None, speak=None) -> str:
    ...
    if file_type == "unknown":
        try:
            ...
            response = model.generate_content(prompt)
            return _clip(_safe_text(response) or "No description returned.")
        except Exception as e:
