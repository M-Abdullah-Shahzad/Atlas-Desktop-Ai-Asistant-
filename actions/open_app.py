...
def open_app(
    ...
    # Path / drive fallback — open in file manager instead of Start Menu search
    import re
    from pathlib import Path as _P
    lower = app_name.lower().strip()
    if (
        re.match(r"^[a-zA-Z]:[/\\]?$", app_name.strip())
        or re.match(r"^[a-zA-Z]\s*drive$", lower)
        or (len(app_name) >= 2 and (":\\" in app_name or app_name.startswith("/") or app_name.startswith("~")))
        or _P(app_name).expanduser().exists()
    ):
        try:
            from actions.file_controller import open_path
            return open_path(app_name)
        except Exception as e:
