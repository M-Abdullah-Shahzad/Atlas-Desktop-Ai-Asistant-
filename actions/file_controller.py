...
def _last_created_path_file() -> Path:
    return Path(__file__).resolve().parent.parent / "memory" / "last_created_path.json"


def _remember_created_path(target: Path) -> None:
    try:
        state_path = _last_created_path_file()
        state_path.parent.mkdir(parents=True, exist_ok=True)
        resolved = target.expanduser().resolve(strict=False)
        state_path.write_text(
            json.dumps({"path": str(resolved)}, ensure_ascii=False),
            encoding="utf-8",
        )
    except Exception:
        pass
...
def _load_last_created_path() -> Path | None:
    try:
        state_path = _last_created_path_file()
...
def open_path(path: str, name: str = "") -> str:
    ...
    try:
        if not path and not name:
            last = _load_last_created_path()
            if last:
                target = last
            else:
                target = _get_desktop()
        else:
            base = _resolve_path(path or "desktop")
