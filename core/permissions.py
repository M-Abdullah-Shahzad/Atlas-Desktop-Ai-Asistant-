...
def get_confirm_timeout() -> float:
    try:
        timeout = float(_load_settings().get("confirm_timeout_seconds", 120))
    except (TypeError, ValueError):
        return float(_DEFAULTS["confirm_timeout_seconds"])
...
def classify_user_permission_reply(text: str) -> str | None:
    ...
    if any(token in _DENY_EXACT for token in tokens):
        return "deny"

    # Deny first (prefer abort over accidental grant)
    for p in _DENY_PHRASES:
        if p in ("don't", "do not"):
            continue
        boundary = rf"(?<!\w){re.escape(p)}(?!\w)"
        if re.search(boundary, cleaned) or re.search(boundary, raw_lower):
            return "deny"
    if re.search(
        r"(?<!\w)(?:don't|do not)\s+(?:do|proceed|continue|send|run|delete|allow|approve|go ahead)(?!\w)",
        cleaned,
    ):
        return "deny"
    ...
    if len(content) <= 4:
        ...
        if len(tokens) >= 2:
            ...
            if any(g in joined for g in ("okay", "goahead", "tryagain", "permissiongranted")):
                return "grant"
            if joined in {"abort", "cancel", "nevermind"}:
                return "deny"
...
def _computer_settings_risk(action: str, description: str = "") -> RiskLevel:
    ...
    explicit_action = re.search(
        r"(?<!\w)(?:shutdown|restart|sleep)(?!\w)",
        a.replace("_", " "),
    )
    actionable_description = re.search(
        r"(?<!\w)(?:shutdown|restart|sleep)\s+(?:the|this|computer|pc|machine|system)(?!\w)",
        desc,
    )
    if explicit_action or actionable_description:
        return RiskLevel.HIGH
    return RiskLevel.LOW
...
def classify_risk(tool_name: str, args: dict[str, Any] | None = None) -> RiskLevel:
    ...
    if name in ("auto_reply_gmail", "set_gmail_vacation_responder"):
        return RiskLevel.MEDIUM

    if name == "initiate_whatsapp_call":
        return RiskLevel.LOW
...
def evaluate_permission(
    ...
    if risk == RiskLevel.CRITICAL:
        denied = _setting_bool(settings, "deny_critical", True)
    ...
    if risk == RiskLevel.LOW:
        auto = _setting_bool(settings, "auto_execute_low_risk", True)
        return PermissionDecision(
            allowed=True,
    ...
    if risk == RiskLevel.MEDIUM:
        need = _setting_bool(settings, "confirm_medium_risk", True)
        return PermissionDecision(
            allowed=True,
    ...
    need = _setting_bool(settings, "confirm_high_risk", True)
    return PermissionDecision(
        allowed=True,
        requires_confirmation=need,
        risk_level=risk,
