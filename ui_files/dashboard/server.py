        ...
        async def auto_login(key: str = ""):
            ...
            if not key or key not in self._pending_keys or self._pending_keys[key] <= now:
                return HTMLResponse(
                    ...
                    f"<p>Press <strong style=\"color:#dde3ed\">Remote Control</strong> in {_assistant_name()} to get a new QR code.</p>"
                    "</div></body></html>"
                )
...
<body>
<script>
  sessionStorage.setItem('Athena_token','{tok}');
  sessionStorage.setItem('Athena_key','{key}');
  localStorage.setItem('Athena_device_token','{dev_tok}');
...
<p>Connecting to {_assistant_name()}…</p>
    ...
    async def serve(self) -> None:
        ...
        print(f"[Dashboard] Press 'Remote Control' in {_assistant_name()} UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
