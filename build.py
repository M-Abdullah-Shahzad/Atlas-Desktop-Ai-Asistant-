"""
Build a standalone Atlas folder: dist/Atlas/Atlas.exe
...
SRC_PNG = ROOT / "config" / "astra.png"
SRC_ICO = ROOT / "config" / "astra.ico"
...
def _stage_runtime_files() -> None:
    ...
    if SRC_PNG.exists():
        _copy_tree(SRC_PNG, DIST / "config" / "astra.png")
    if SRC_ICO.exists():
        _copy_tree(SRC_ICO, DIST / "config" / "astra.ico")
    ...
    if readme_src.is_file():
        ...
        (dest_contacts / "README.txt").write_text(
            ...
            "Settings → WhatsApp Setup → Import, or restart Atlas.\n",
            encoding="utf-8",
        )
    ...
    wa_setup.write_text(
        "Atlas — WhatsApp setup (new users)\n"
        "====================================\n\n"
        "1. Start Atlas.exe and enter a Gemini API key if asked.\n"
    ...
    readme.write_text(
        "Atlas desktop build\n"
        "====================\n\n"
        "Double-click Atlas.exe to start.\n"
        ...
        "Atlas.exe so spoken names resolve to WhatsApp numbers.\n"
        "API keys and WhatsApp sessions are stored next to Atlas.exe\n"
...
def _unlock_dist() -> None:
    """Stop packaged Atlas/Node so PyInstaller can replace dist/Atlas."""
    ...
    if not victims:
        return
    print(f"Stopping {len(victims)} process(es) using dist/Atlas…")
    for proc in victims:
        try:
            proc.terminate()
        except Exception:
...
def main() -> int:
    ...
    if not (DIST / "Atlas.exe").exists() and not (DIST / "Atlas").exists():
    ...
    print(f"  Launch: {exe}")
    print("  Zip the whole dist/Atlas folder to share it.")
